...
pub fn analyze(
    ...
    let mut smallest_x = 12345;

    let mut all_x = vec![];
    for r in rects.iter() {
        dbg!(r);
        if r.from.x != 0 {
            smallest_x.update_min(r.from.x);
        }
        all_x.push(r.from.x);
    }
    all_x.sort();
    ...
    let pixel_dist = get_pixel_distance(&my_current, &test_case.expected);

    dbg!(pixel_dist);

    let new_rects = restore_rects_by_field(&my_current, test_case);
    ...
    let mut new_target = Array2D::new(Color::default(), n, m);

    let change_x = |x: usize| -> usize {
        if x >= start_x && x < end_x {
            new_start_x + (x - start_x)
        } else if x >= new_start_x && x < start_x {
            x + (end_x - start_x)
        } else {
            x
        }
    ...
    for x in 0..n {
        for y in 0..n {
            let nx = change_x(x);
            new_target[nx][y] = test_case.expected[x][y];
        }
    }
    let mut new_rects = vec![];
    for r in rects.iter() {
        new_rects.push(SolutionRect {
            to: r.to,
            color: r.color,
            from: p(change_x(r.from.x as usize), r.from.y as usize),
        });
    }
    ...
    let new_testcase = TestCase {
        expected: new_target,
        start_field: test_case.start_field.clone(),
